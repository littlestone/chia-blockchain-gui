...
import mintEnvironment from '../util/mintEnvironment';
import mintConfig from '../util/config';
...
if (!handleSquirrelEvent()) {
  ...
  const ensureCorrectEnvironment = () => {
    ...
    if (!mintEnvironment.guessPackaged() && !('VIRTUAL_ENV' in process.env)) {
  ...
  if (ensureSingleInstance() && ensureCorrectEnvironment()) {
    ...
    const createWindow = async () => {
      ...
      if (!mintConfig.manageDaemonLifetime()) {
      ...
      mainWindow.on('close', (e) => {
        ...
        if (decidedToClose || !mintConfig.manageDaemonLifetime()) {
          return;
        }
        e.preventDefault();
        if (!isClosing) {
    ...
    const appReady = async () => {
      ...
      // if the daemon isn't local we aren't going to try to start/stop it
      if (mintConfig.manageDaemonLifetime()) {
        mintEnvironment.startMintDaemon();
      }
    };
  ...
  const getMenuTemplate = () => {
    const template = [
      ...
      },
      {
        ...
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'Mint Blockchain Wiki' }),
            click: () => {
              openExternal(
                'https://github.com/MintNetwork/mint-blockchain/wiki',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/MintNetwork/mint-blockchain/wiki/FAQ',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/MintNetwork/mint-blockchain/releases',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/MintNetwork/mint-blockchain/blob/main/CONTRIBUTING.md',
              );
            },
          },
          {
          ...
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/MintNetwork/mint-blockchain/issues',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal('https://discord.gg/rAtQwUJ9Ac');
            },
          },
          {
            label: i18n._(/* i18n */ { id: 'Follow on Twitter' }),
            click: () => {
              openExternal('https://twitter.com/MintBlockchain');
            },
          },
        ],
      },
    ...
    if (process.platform === 'darwin') {
      // Mint Blockchain menu (Mac)
      template.unshift({
        label: i18n._(/* i18n */ { id: 'Mint' }),
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'About Mint Blockchain' }),
            click: () => {
              openAbout();
            },
          },
