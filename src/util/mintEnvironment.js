...
const getMintVersion = () => {
  let version = null;
  const exePath = getExecutablePath('mint');
  // first see if we can get a mint exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or mint is on the path
    try {
      version = child_process
