...
export default function Block() {
  ...
  const poolReward = mojo_to_mint(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojo_to_mint(
    calculateBaseFarmerReward(blockRecord.height),
  );

  const mintFees = blockRecord.fees
    ? mojo_to_mint(BigInt(blockRecord.fees))
    : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://alltheblocks.net/mint/block/${blockRecord.header_hash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: mintFees ? `${mintFees} ${currencyCode}` : '',
      tooltip: (
        <Trans>
    ...
    <LayoutMain title={<Trans>Block</Trans>}>
      <Card
        title={
          <Back variant="h5">
            <Trans>
              Block at height {blockRecord.height} in the Mint blockchain
            </Trans>
          </Back>
        }
        action={
