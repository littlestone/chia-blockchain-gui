...
import { AlertDialog, Card, Flex } from '@mint/core';
...
function BalanceCard(props: BalanceCardProps) {
  ...
  const balance_ptotal_mint = mojo_to_colouredcoin_string(balance_ptotal);
  const balance_pending_mint = mojo_to_colouredcoin_string(balance_pending);
  const balance_change_mint = mojo_to_colouredcoin_string(balance_change);
  const acc_content =
    balancebox_1 +
    balancebox_2 +
    balancebox_ptotal +
...
function SendCard(props: SendCardProps) {
  ...
  function send() {
    ...
    if (address.includes('mint_addr') || address.includes('colour_desc')) {
      dispatch(
        openDialog(
          <AlertDialog>
